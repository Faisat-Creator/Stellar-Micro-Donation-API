...
- [x] 1. Audit and document current header emission behavior
...
- [x] 2. Consolidate header builder logic
  - [x] 2.1 Verify `buildRateLimitHeaders` in `src/middleware/rateLimitHeaders.js` meets requirements
  ...
  - [x] 2.2 Write property test for buildRateLimitHeaders format consistency
    - **Property 6: Header builder returns consistent format**
    - **Validates: Requirements 3.1**
  
  - [x] 2.3 Write property test for negative remaining clamping
    - **Property 7: Negative remaining clamped to zero**
    - **Validates: Requirements 2.3**
  
  - [x] 2.4 Update `perKeyRateLimit.js` to use centralized header builder
...
- [x] 3. Standardize Retry-After header emission
  - [x] 3.1 Create helper function for Retry-After calculation
  ...
  - [x] 3.2 Update all express-rate-limit handlers to use Retry-After helper
  ...
  - [x] 3.3 Update perKeyRateLimit to use Retry-After helper
    - Import and use `calculateRetryAfter` in 429 handler
    - _Requirements: 1.3, 4.1, 4.2, 4.3_

- [x] 4. Verify express-rate-limit configuration
  - [x] 4.1 Audit all limiters for standardHeaders and legacyHeaders config
  ...
  - [x] 4.2 Verify express-rate-limit emits headers on successful responses
...
- [x] 6. Add integration tests for header emission
  - [x] 6.1 Extend `tests/middleware/rate-limiter.test.js`
  ...
  - [x] 6.2 Write property test for successful response headers
    - **Property 1: All rate-limited responses include standard headers**
    - **Validates: Requirements 1.1, 1.4**
  
  - [x] 6.3 Write property test for 429 response headers
    - **Property 2: 429 responses include Retry-After**
    - **Validates: Requirements 1.3, 4.1, 4.3**
  
  - [x] 6.4 Write property test for IETF and legacy header matching
    - **Property 3: IETF and legacy headers match**
    - **Validates: Requirements 1.4**

- [x] 7. Add per-key rate limiter header tests
  - [x] 7.1 Extend `tests/middleware/per-key-rate-limit.test.js`
  ...
  - [x] 7.2 Write property test for header value accuracy
    - **Property 4: Header values reflect limiter state**
    - **Validates: Requirements 2.3**
  
  - [x] 7.3 Write property test for Retry-After alignment
    - **Property 5: Retry-After aligns with reset time**
    - **Validates: Requirements 4.2**

- [x] 8. Test coverage for all rate limiter types
  - [x] 8.1 Create test helper for header verification
  ...
  - [x] 8.2 Add header tests for donation rate limiter
    - Test successful response headers
    - Test 429 response headers
    - _Requirements: 1.1, 1.2, 1.3, 1.4_
  
  - [x] 8.3 Add header tests for verification rate limiter
    - Test successful response headers
    - Test 429 response headers
    - _Requirements: 1.1, 1.2, 1.3, 1.4_
  
  - [x] 8.4 Add header tests for batch rate limiter
    - Test successful response headers
    - Test 429 response headers
    - _Requirements: 1.1, 1.2, 1.3, 1.4_
  
  - [x] 8.5 Add header tests for bulk import rate limiter
    - Test successful response headers
    - Test 429 response headers
    - _Requirements: 1.1, 1.2, 1.3, 1.4_
  
  - [x] 8.6 Add header tests for auth token rate limiter
    - Test successful response headers
    - Test 429 response headers
    - _Requirements: 1.1, 1.2, 1.3, 1.4_
  
  - [x] 8.7 Add header tests for stats rate limiter
    - Test successful response headers
    - Test 429 response headers
    - _Requirements: 1.1, 1.2, 1.3, 1.4_

- [x] 9. Add regression tests for preserved behavior
  - [x] 9.1 Test rate limit thresholds unchanged
